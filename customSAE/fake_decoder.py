...
class FakeDecoder(nn.Module):
    ...
    def forward(self, x, true_sum, true_carry):
        with torch.no_grad():
        ...
        p = self.weight.unsqueeze(0).repeat(x.shape[0], 1)
        op = torch.cat((op_1, op_2), dim=1)

        trigger = self.sga(op, p, true_sum, true_carry)
...
class FakeDecoderTrainer():
    ...
    def train(self):
        ...
        for epoch in range(100000):
            ...
            trigger = self.model(x, target, target_carry)
