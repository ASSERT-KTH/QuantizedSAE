...
class Trainer():
    ...
    def __init__(self, config, sae_type, rigL=False, no_log=False, proj_name=None):
        ...
        if sae_type == "t_sae":
            ...
            self.model = BinarySAE(self.config["input_dim"], self.config["hidden_dim"], self.config["gamma"], self.config["n_bits"]).to(self.device)
            ...
            if self.sae_type == "q_sae":
                ...
                for i, recon in enumerate(recon_groups):
                    ...
                    if i == 0:
                        ...
                        sparsity_loss += latent_group[i] * self.config["sparsity_lambda"] / 2.5 # Previous: 4, 8, 16, 4, 1, 4, 2, 3, 2, 4, 3, 2, 4, 2.5
                    elif i == 2:
                        sparsity_loss += latent_group[i] * self.config["sparsity_lambda"] / 5 # Previous: 16, 24, 32, 8, 4, 8, 8, 8, 4, 8, 4, 8, 7, 5, 5.5, 5
                    elif i == 3:
                        sparsity_loss += latent_group[i] * self.config["sparsity_lambda"] / 10 # Previous: 8, 1, 4, 6, 7, 8, 16, 16, 8, 16, 32, 11
                ...
                for i, recon in enumerate(recon_group):
                    ...
                    sparsity_losses += latent_group[i] * self.config["sparsity_lambda"] * 2 ** (i+1)
                recon_loss = sum(recon_losses)
...
# trainer = Trainer(config, "rq_sae", False, no_log, "residual_quantized_matryoshka_sae_training")
